...
\chapter{Static Noninteracting Polarizability in Time-Reversal Symmetrical insulators}
\label{app:polarizability_TRS_proof}

In this section we derive expression~\eqref{eq:static_polarizability_TRS} from section~\ref{subsec:non_interacting_polarizability} from its more general form~\eqref{eq:static_polarizability_repeated}, assuming the system is time-reversal symmetric.
...
\begin{equation}
\begin{aligned}
     \chi^0_{\bG\bG'}(\bq) = &\sum_{vc,\bk \sigma} \Bigg[\frac{\langle c,\bk|\ee^{-\ii(\bq+\bG)\cdot\br}|v,\bk+\bq\rangle\langle v,\bk+\bq|\ee^{\ii(\bq+\bG')\cdot\br}|c,\bk\rangle}{\epsilon_{v,\bk+\bq}-\epsilon_{c,\bk}} \Bigg] + \\
     & \sum_{vc,\bk \sigma} \Bigg[\frac{\langle v,\bk|\ee^{-\ii(\bq+\bG)\cdot\br}|c,\bk+\bq\rangle\langle c,\bk+\bq|\ee^{\ii(\bq+\bG')\cdot\br}|v,\bk\rangle}{\epsilon_{v,\bk} - \epsilon_{c,\bk+\bq}}\Bigg] \,.
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
    \langle n,\bk | \ee^{-\ii(\bk' - \bk + \bG) \cdot \br} | n',\bk' \rangle &=  (\bC^{n,\bk})^\dagger \cdot \big(\bC^{n',\bk'} \odot \underline{\varphi} \big) =  \Big((\bC^{n,-\bk})^\intercal T^\dagger \Big) \cdot \Big( \big(T (\bC^{n',-\bk'})^* \big) \odot \underline{\varphi} \Big) = \\
    &= (\bC^{n,-\bk})^\intercal (T^\dagger  T) \Big((\bC^{n',-\bk'})^* \odot \underline{\varphi}\Big) = (\bC^{n',-\bk'})^\dagger \cdot \big(\bC^{n,-\bk} \odot \underline{\varphi}\big) = \\
    &= \langle n',-\bk' | \ee^{-\ii(\bk' - \bk + \bG) \cdot \br} | n,-\bk \rangle \,, \qq{with $\intercal$ denoting transpose}
\end{aligned}
\end{equation}
