...
\chapter{Support information for Chapter 5}
\label{app:hBN_superlattice}

This appendix serves as supplemental material for the work in Chapter~\ref{ch:Optical_hBN_superlattice} of the main text.
Just like in that chapter, the contents of this appendix are adapted from the publication in Ref.~\cite{Ninhos_Tserkezis_Mortensen_Peres_2024}.


\section{Recovering the original Hamiltonian}
\label{sec:recover_original_hamiltonian}

To conciliate the renormalized low-energy Hamiltonian~\eqref{eq:new_Dirac_Hamiltonian} with the original one~\eqref{eq:low_energy_hamiltonian_hBN_K}, we take in the former the two limits $V_0 \to 0$ and $L \to \infty$ separately.
In both cases, we should recover the latter.
In this appendix, we choose for the momentum $q$ as in the Chapter~\ref{ch:quantum_mechanics_solids}, instead of $k$ as in the Chapter~\eqref{ch:Optical_hBN_superlattice}.
...
The result for $m \neq 0$ is not null, but we can interpret the two terms on the right-hand side as follows.
The first term is purely a global shift in energies resultant from shifting the momentum along the $x$ direction by $m G_0/2$, a consequence of using basis states with the spatial dependent part of the form $\exp{\ii (\bq \pm m G_0 \uvec{x}/2)}$ used when deriving the renormalized low-energy Hamiltonian.
...
\section{Exchange energy correction to the bandgap}
...
\begin{equation}
    \begin{split}
        E_\nu \frac{1}{\sqrt{A}} \phi_\nu(\bk)  =&   \phi_{\nu}(\bk) (E_{\bk,c}-E_{\bk,v}) - \frac{1}{\area} \sum_{\bp} \phi_{\nu} (\bp) V_{\bk-\bp} F_{cvvc}(\bk,\bp,\bk-\bp) +  \\
        & + \frac{1}{\area} \phi_{\nu}(\bk) \sum_{\bp} V_{\bp} \left[F_{vvvv}(\bp + \bk ,\bk,\bp) - F_{vcvc}(\bp + \bk,\bk,\bp) \right].
    \end{split}
\end{equation}

\noindent We wish to compute the contribution to the bandgap due to the exchange energy coming from the third term in the right-hand side of the previous equation.
We assume that this term renormalizes strongly the electronic bandgap but has no effect on the curvature of the bands.
So we start from that term with $\bk = 0$\,.
We denote the exchange energy by $\Delta_{\mathrm{ex}}$ and it writes

\begin{equation}
    \Delta_{\mathrm{ex}} = \frac{1}{\area} \sum_{\bp} V_{\bp} \left[F_{vvvv}(\bp ,0,\bp) - F_{vcvc}(\bp,0,\bp) \right] \,.
\end{equation}
...
\noindent The overlap of the spinors (see Eqs.~\eqref{eq:valence_spinor} and~\eqref{eq:conduction_spinor}) reads
...
Using the auxiliary function $\gamma(\theta) = \sqrt{\cos^2 \theta + J_0^2(\beta) \sin^2 \theta}$\,, the exchange energy now reads

\begin{equation}
    \Delta_{\mathrm{ex}} = \frac{1}{\area} \sum_{\bp} V_{\bp}  \frac{1}{\sqrt{1 + \left(\frac{p \gamma(\theta)}{k_0}\right)^2}} = \frac{e}{8 \pi^2 \epsz \kappa} \int_0^{2\pi} \dd \theta \int_0^{\infty} \dd p  \frac{1}{ (1 + r_0 p/\kappa) \sqrt{1 + \left(\frac{p \gamma(\theta)}{k_0}\right)^2}} \,.
\end{equation}

\noindent where we converted the sum into an integral as $\sum_{\bp} \to \area/(2 \pi)^2  \int \dd \bp$, and used for the Rytova--Keldysh potential

\begin{equation}
    V_{\bp} = \frac{e}{2 \epsz p (\kappa + r_0 p)} \,.
\end{equation}

\noindent with $\kappa$ being the arithmetic average between the dielectric constants of the substrate and the cladding material.
