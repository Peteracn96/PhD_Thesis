...
But it is well-known in the literature, that when dealing with pair-wise particle interactions, screening has to be taken into account~\cite{Latini_2015,Cudazzo_Tokatly_Rubio_2011}. \textcolor{blue}{(Ignoring screening is actually one of the pit-falls of the \hf~ method.)}
...
Here we make the extension of the code to go beyond thew Rytova-Keldysh approximation to compute the interaction matrix elements to build the Bethe-Salpeter Hamiltonian and solve the Bethe-Salpeter equation.}
...
\noindent where $v_c$ denotes the bare (without screening) Coulomb potential and $\chi^0 (\br',\br'';t-t')$ is the \rpa~irreducible polarizability, for which we have assumed time translation invariance.
When Fourier transformed from the time domain to the frequency domain, and from real space to reciprocal space, it has the following form:
...
The quantity $\chi^0_{\bG \bG'} (\bq,\omega)$ is the noninteracting irreducible polarizability, that as per~\eqref{eq:polarizability_momentum_frequency} reads
...
Since we are interested in electrostatic screening for \bd~insulators and semiconductors (from now on, let us just say insulators and account for all \bd~gapped materials), we replace the volume $\vol$ with the area $A$.
We consider as well zero temperature, so that we can simplify the polarizability to

\begin{equation}
    \label{eq:static_polarizability_repeated}
    \begin{aligned}
        \chi^0_{\bG\bG'}(\bq,\omega \to 0) = & \frac{1}{A} \sum_{v c,\bk \sigma} \Bigg[\frac{\langle c,\bk| \ee^{-\ii(\bq+\bG)\cdot\br}|v,\bk+\bq\rangle \langle v,\bk+\bq| \ee^{\ii(\bq+\bG')\cdot\br}|c,\bk\rangle}{\epsilon_{v,\bk+\bq}-\epsilon_{c,\bk}} \Bigg] + \\
        & \frac{1}{A} \sum_{v c,\bk \sigma} \Bigg[\frac{\langle v,\bk| \ee^{-\ii(\bq+\bG)\cdot\br}|c,\bk+\bq\rangle\langle c,\bk+\bq| \ee^{\ii(\bq+\bG')\cdot\br}|v,\bk\rangle}{\epsilon_{v,\bk} - \epsilon_{c,\bk+\bq}}\Bigg] \,.
    \end{aligned}
\end{equation}

\noindent In the previous expression, if the system enjoys \gls{trs}, then the two terms in the \gls{rhs} add up so that we can write
...
We would like to take this opportunity to make a brief remark.
...
\noindent The matrix elements of interest therefore write
...
Note that even though the notation is similar to that of Ref.~\cite{Xatu}, the definition for $I^{\bG}_{n \bk, n' \bk'}$ is not exactly the same.
...
\noindent As we will see later on, this poses a huge advantage when compared to \abinit~3D approaches, where the unscreened Coulomb potential is $v_c(\bq) = e/\epsz |\bq|^2$, and in particular when we have to compute the dielectric matrix at the origin.
...
\noindent where $\chi^{\RPA}_{\bG \bG'} (\bq)$ is the interacting polarizability within \rpa, defined by the Dyson equation~\cite{Bruus_Flensberg_2004}
...
Within \rpa, one ignores exchange and correlation, which is why the interaction kernel coincides with the bare Coulomb potential~\cite{Cohen_Louie_2016,Bruus_Flensberg_2004}, accounting only for the direct interaction.
