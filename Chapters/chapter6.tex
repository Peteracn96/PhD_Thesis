...
In particular, when we have to compute the dielectric matrix at the origin.
As we will see, the polarizability matrix element $\chi^0_{\bzero \bzero} (\bq)$ approaches zero quadratically in $|\bq| = q$.
...
To compute the dielectric matrix and invert it, we have no off-plane contribution to the reciprocal lattice vectors, so the number of matrix elements to compute, at each point $\bq \in \BZ$, scales with $N^2$ instead of $N^3$.
...
\noindent when written in a compact form, where $V_c$ is the interaction kernel.
Within \rpa, one ignores exchange and correlation, that is why the interaction kernel coincides with the bare Coulomb potential~\cite{Cohen_Louie_2016,Bruus_Flensberg_2004}, accounting only for the direct interaction.
In the previous equation, all quantities, when written in momentum space, are infinite matrices as well, so we would need to apply a cutoff for the vectors $\bG$ either way.
...
If the band gap that, estimated by $ \epsilon_{n',\bp} - \epsilon_{n,\bp}$ (assuming it is direct and located at the point $\bq$ in the \bz) is large enough when compared to the frequency, then we can approximate the screened Coulomb potential by its static limit, and in the interaction matrix elements discard an integration in frequency.
...
To preserve basic symmetries and degeneracies, we hope that $G_{c,X}$ can be made small enough compared to $G_c$\footnote{In practice, we always have $G_{c,X} < G_c$, since we have to compute the dielectric matrix before computing the exciton.}.
...
\subsection{\gls{hbn} revisited: \gls{dft} band structure and Hamiltonian model}
...
The hybrid functional HSE06\todo{need a Ref maybe} was used with the Perdew--Burkr--Ernzerhof functional within the \gls{gga}, with a $48\times48\times1$ $k$-grid.
