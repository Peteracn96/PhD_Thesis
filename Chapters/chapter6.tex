...
\noindent which depends on momentum as $~1/q$, unlike its \gls{3d} counterpart that goes as $~1/q^2$.
As we will see later on, this will put us in advantage over typical first-principle approaches.
...
The reason why we implement the symmetrized version of the dielectric function is that it simplifies the treatment of the numerical divergences in the dielectric function, and subsequently in the screened potential, in the long-wavelength limit~\cite{Baldereschi_Tosatti_1978,Rasmussen_Schmidt_Winther_Thygesen_2016,Shishkin_Kresse_2006}.
...
The results have to be converged by increasing $L_{\perp}$, and only in the limit of infinite separation $\varepsilon_M (\bzero) = 1$ for a \bd~material.
...
Issues of these kind are routine in the literature~\cite{Berkeley_GW,Louie_2000,Rasmussen_Schmidt_Winther_Thygesen_2016,Guandalini_Varsano_2023}.
