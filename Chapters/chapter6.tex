...
For this model, we have obtained a bandgap of $\Delta = 6.08$ \unit{\electronvolt}, not very distant from the one reported in Section~\ref{subsec:band_gap_and_binding_energies}.
...
We just advert the reader for the fact that the polarizability presented in the figure and the following ones is multiplied by the unit cell area $\area_{\UC}$, as in our implementations we cast the area of the unit cell to the Coulomb potential and left the factor of $1/N$ in the polarizability for it to converge with the size of the \bz~mesh.
...
\begin{itemize}
    \item does not depend on the direction of $\bq$, but only on $q$. This isotropic behavior is expected for \gls{hbn}, considered an isotropic material.
...
\begin{figure}[H]
    ...
    \mycaption[Polarizability in the \bz~mesh: an off-diagonal element]{Matrix element $\chi^0_{\bG_1 \bzero} (\bq)$ of the polarizability in the \bz~for \gls{hbn}. Panel \textbf{a} displays the real part of the polarizability, while panel \textbf{b} displays the imaginary part. The bar legend for the colors are inside the respective plots.}
